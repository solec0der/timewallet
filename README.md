...
| Spend session | Deducts 2× minutes, unshields, `DeviceActivity` usage threshold re-shields after the bought minutes are *actually used* (valid until midnight) |
...
- Buying scroll time within ~15 min of midnight is rejected (DeviceActivity
  minimum interval) — you get refunded.
- Screen Time API quirk: unshielded minutes are metered by usage, so unused
  bought minutes stay available until midnight.
